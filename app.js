...
function noticePdf(c){const company=COMPANIES[c.fd.companyKey];return `<h1 class="doc-subtitle">${esc(company.name)}</h1><h2 class="doc-title">員 工 資 遣 通 知 書</h2><table class="doc-table paired-form"><colgroup><col class="label"><col class="value"><col class="label"><col class="value"></colgroup><tr class="section-row"><th colspan="4">一、員工資料</th></tr><tr><th>姓名</th><td>${esc(c.fd.employeeName)}</td><th>身分證字號</th><td>${esc(c.fd.employeeId)}</td></tr><tr><th>通訊地址</th><td>${esc(c.fd.employeeAddress)}</td><th>聯絡電話</th><td>${esc(c.fd.employeePhone)}</td></tr><tr><th>部門</th><td>${esc(c.fd.department)}</td><th>職稱</th><td>${esc(c.fd.jobTitle)}</td></tr><tr class="section-row"><th colspan="4">二、資遣資料</th></tr><tr><th>預告期</th><td>${c.noticeDays}日</td><th>最晚預告日</th><td>${c.latestNotice?roc(c.latestNotice):'無'}</td></tr><tr><th>引用條款</th><td colspan="3">${esc(c.fd.legalBasis)}</td></tr><tr><th>理由</th><td colspan="3">${esc(c.fd.reasonDetail)}</td></tr><tr><th>到職日</th><td>${roc(c.start)}</td><th>總年資</th><td>${c.tenure.totalDays}日</td></tr><tr><th>離職日</th><td>${roc(c.end)}</td><th>薪資發放日</th><td>${roc(c.fd.settlementDate)}</td></tr><tr class="section-row"><th colspan="4">三、資遣費金額</th></tr><tr><th>平均工資</th><td class="right">${Math.round(c.monthly).toLocaleString()} 元</td><th>資遣費</th><td class="amount-box">${c.severance.toLocaleString()} 元</td></tr></table><div class="notice-statement"><b>說明：</b><br>離職同仁應遵守保密協定之相關規定，並且不挪用或損毀公司財物與資料、不持有、洩漏或使用因職務獲取之資料及秘密，若有違反公司將請求賠償並應負擔相關法律責任。</div><table class="doc-table approval-table"><tr class="section-row"><th colspan="5">四、簽核欄位</th></tr><tr><th>核准</th><th>會計</th><th>人資</th><th>主管</th><th>員工</th></tr><tr><td></td><td></td><td></td><td></td><td></td></tr></table><p class="doc-note">資遣費遇小數點無條件進位至整數元。</p>`}
function averagePdf(c){const company=COMPANIES[c.fd.companyKey];const rows=c.wages.map(w=>`<tr><td>${esc(w.period)}</td><td class="right">${Math.round(w.original).toLocaleString()}</td><td class="right">${Math.round(w.partialDeduction).toLocaleString()}</td><td class="right">${Math.round(w.leaveDeduction).toLocaleString()}</td><td class="right">${Math.round(w.lateDeduction).toLocaleString()}</td><td class="right">${Math.round(w.otherExclude).toLocaleString()}</td><td class="right">${Math.round(w.net).toLocaleString()}</td></tr>`).join('');return `<h1 class="doc-subtitle">${esc(company.name)}</h1><h2 class="doc-title">平 均 工 資 計 算 明 細</h2>${docTable([['員工姓名',c.fd.employeeName,'計算類型',c.fullSix?'滿六個月':'未滿六個月'],['到職日',roc(c.start),'離職日',roc(c.end)],['總年資',`${c.tenure.totalDays}日`,'部門／職稱',`${c.fd.department}／${c.fd.jobTitle}`]])}<table class="doc-table wage-detail" style="margin-top:16px"><tr><th>年／月</th><th>原工資</th><th>未足月扣款</th><th>事假扣款</th><th>遲到扣款</th><th>其他排除</th><th>應領工資</th></tr>${rows}<tr><th colspan="6">總應領工資</th><td class="right">${Math.round(c.totalNet).toLocaleString()}</td></tr></table><div class="formula-box"><b>平均工資算式</b><br>${c.fullSix?`${Math.round(c.totalNet).toLocaleString()} ÷ 6個月`:`${Math.round(c.totalNet).toLocaleString()} ÷ ${c.tenure.totalDays}日 × 30`} ＝ <b>${Math.round(c.monthly).toLocaleString()} 元</b><br><span class="doc-note">未足月、事假及遲到扣款的小數均無條件捨去。</span></div><table class="doc-table approval-table"><tr><th>核准</th><th>會計</th><th>人資</th><th>主管</th><th>員工</th></tr><tr><td></td><td></td><td></td><td></td><td></td></tr></table>`}
function servicePdf(c){const company=COMPANIES[c.fd.companyKey];return `<h1 class="doc-title">離 職 證 明 單</h1>${docTable([['姓名',c.fd.employeeName,'出生年月日',rocLong(c.fd.birthDate)],['性別',c.fd.gender==='female'?'女':'男','身分證字號',c.fd.employeeId],['服務部門',c.fd.department,'職稱',c.fd.jobTitle],['到職日期',rocLong(c.start),'',''],['離職日期',rocLong(c.end),'','']])}<p style="text-align:center;margin:24px 0">上列各項確實無訛　特此證明</p><div class="company-block">公司名稱：${esc(company.name)}　　　　　　　　　（公司章）<br>營利事業登記證字號：${esc(company.id)}<br>地址：${esc(c.fd.workplace)}<br>負責人：${esc(company.representative)}　　　　　　　　　（簽章）<br>電話：${esc(company.phone)}</div><p style="text-align:center;margin-top:35px">${rocLong(new Date())}</p>`}
